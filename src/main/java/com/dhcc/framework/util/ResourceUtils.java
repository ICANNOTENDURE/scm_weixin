...
public class ResourceUtils {
	...
	public static InputStream getInputStream(String path,Class clasz) throws IOException {
//		if (path.startsWith("\\")) {
//			path = path.substring(1);
//		}
		InputStream is = null;
		is =clasz.getResourceAsStream(path);
		if (is == null) {
			throw new FileNotFoundException(
					" cannot be opened because it does not exist");
		}
	...
	public static List getSpeciDirFileNamesByFiles(String dir,
		...
		for (int i = 0; i < tempFiles.length; i++) {
			if (tempFiles[i].getName().indexOf(".xml") != -1) {
