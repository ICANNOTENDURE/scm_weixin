...
(function($)
{
	...
	var methods = {
		init : function(options) {
			...
			return this.each(function () {
				...
				if(!originalName) originalName = $(this).attr("id");
				var	nameDx = originalName + "ms2side__dx";
				var	nameSx = originalName + "ms2side__sx";
				var size = $(this).attr("size");
				size = parseInt(size);
				size = isNaN(size)? 6:size;
				o.minSize = parseInt(o.minSize);
				o.minSize = isNaN(o.minSize)? 6:o.minSize;
				// SIZE MIN
				if (size < o.minSize) {
				...
				var htmlToAdd = 
					"<div class='ms2side__div'>" +
						...
						"<div class='ms2side__options'>" +
							...
							?
							("<p class='AddOne' title='添加选中项'>"+o.labelRight+"</p>" +
							"<p class='AddAll' title='添加所有'>"+o.labelRightRight+"</p>" +
							"<p class='RemoveOne' title='移除选中项'>"+o.labelLeft+"</p>" +
							"<p class='RemoveAll' title='移除所有'>"+o.labelLeftLeft+"</p>")
							:
							("<p class='AddOne' title='添加选中项'>"+o.labelLeft+"</p>" +
							"<p class='AddAll' title='添加所有'>"+o.labelLeftLeft+"</p>" +
							"<p class='RemoveOne' title='移除选中项'>"+o.labelRight+"</p>" +
							"<p class='RemoveAll' title='移除所有'>"+o.labelRightRight+"</p>")
							) +
						"</div>" +
						"<div class='ms2side__select'>" +
	    	    ...
	    	    el.on('change', function() {
	    	      var _options = $(this).data('multiselect2side');
	    	      var maxSelectNums = parseInt(_options.maxSelected);
	    	      maxSelectNums = isNaN(maxSelectNums)? maxSelectNums:-1;
	    		  var selectedS = $(this).find('option:selected');
	    		  if(maxSelectNums >= 0) {
	    			  selectedS.slice(0, maxSelectNums)
	    		  } 
	    		  var unselectedS = $(this).find('option:not(:selected)');
	    		  leftSel.children().remove('option');
	    		  rightSel.children().remove('option');
	    		  unselectedS.clone().appendTo(leftSel);
	    		  selectedS.clone().appendTo(rightSel);
	    	    });
			});
		},
		destroy : function( ) {
		...
		getValue : function() {
			return $CommonUI.getUIValue($(this));
		},
		setValue : function(val) {
			$CommonUI.setUIValue($(this), val);
			$(this).change();
		},
		getOptions : function() {
			...
			for(var i=0;i<opts.length;i++) {
				option.push({'value': opts.eq(i).val(), 'text': opts.eq(i).html(), 'selected': opts.eq(i).prop('selected')});
			}
			return option;
		},
		getSelectedOptions : function() {
			...
			var selected = [];
			for(var i=0;i<opts.length;i++) {
				selected.push({'value': opts.eq(i).val(), 'text': opts.eq(i).html()});
			}
			return selected;
		},
  ...
  $.fn.multiselect2side = function( method ) {
    if ( methods[method] ) {
      ...
      if(!$(this).hasClass('multiselect2side')) {
    	  $(this).addClass('multiselect2side');
      }
