...
$(function (){
	...
	$CommonUI.getDataGrid('#datagrid').datagrid({  
	    url:$WEB_ROOT_PATH+'/ven/venIncAdviceCtrl!list.htm',
	...
	//新增或更新成功的回调函数
	function succ(data){
		var Id=$('#detail input[name="dto.venIncAdvice.venIncAdviceId"]').val();	
		if(Id){
			$CommonUI.alert("修改成功");
		}else{
			$CommonUI.alert("增加成功");
	...
	$("#submitBtn").on('click', function() {
		postReq($WEB_ROOT_PATH+'/ven/venIncAdviceCtrl!save.htm',"#detail",succ,err,{skipHidden:false});
	});
...
function editRow() {
	...
	var Id=row.venIncAdviceId;
	var url = $WEB_ROOT_PATH+'/ven/venIncAdviceCtrl.htm?BLHMI=findById&dto.venIncAdvice.venIncAdviceId='+Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function(){
		...
		$.post($WEB_ROOT_PATH+"/ven/venIncAdviceCtrl!delete.htm", {'dto.venIncAdvice.venIncAdviceId':row.venIncAdviceId},function(){
