...
$(function (){
	...
	$CommonUI.getDataGrid('#datagrid').datagrid({  
	    url:$WEB_ROOT_PATH+'/st/stInGdRecCtrl!list.htm',
	    iconCls:'icon-edit',//图标
	    method:'post',
	    fit:true,
	    columns:[
            [{"title":"ingdrecId","field":"ingdrecId","sortable":false,"hidden":true},{"title":"ingdrecLocId","field":"ingdrecLocId","sortable":false,"hidden":false},{"title":"ingdrecVenId","field":"ingdrecVenId","sortable":false,"hidden":false},{"title":"indrecDate","field":"indrecDate","sortable":false,"hidden":false},{"title":"ingdrecStatus","field":"ingdrecStatus","sortable":false,"hidden":false},{"title":"ingdrecRemark","field":"ingdrecRemark","sortable":false,"hidden":false},{"title":"ingdrecNo","field":"ingdrecNo","sortable":false,"hidden":false}]
	    ]	 
	});
	
	//新增或更新成功的回调函数
	function succ(data){
		var Id=$('#detail input[name="dto.stInGdRec.stInGdRecId"]').val();	
		if(Id){
			$CommonUI.alert("修改成功");
		}else{
			$CommonUI.alert("增加成功");
	...
	$("#submitBtn").on('click', function() {
		...
		postReq($WEB_ROOT_PATH+'/st/stInGdRecCtrl!save.htm',"#detail",succ,err,{skipHidden:false});
	});
...
function editRow() {
	...
	var Id=row.stInGdRecId;
	var url = $WEB_ROOT_PATH+'/st/stInGdRecCtrl.htm?BLHMI=findById&dto.stInGdRec.stInGdRecId='+Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function(){
		...
		$.post($WEB_ROOT_PATH+"/st/stInGdRecCtrl!delete.htm", {'dto.stInGdRec.stInGdRecId':row.stInGdRecId},function(){
