...
$(function() {
	...
	$("#submitBtn").on(
			...
			function() {
				...
				postReq($WEB_ROOT_PATH + '/demo/studentCtrl!save.htm',
						"#detail", succ, err, {
							skipHidden : false
				});
			});
...
function editRow() {
	...
	var url = $WEB_ROOT_PATH
			+ '/demo/studentCtrl.htm?BLHMI=findById&dto.student.studentId='
			+ Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function() {
		...
		$.post(
				$WEB_ROOT_PATH + "/demo/studentCtrl!delete.htm", 
				{
					'dto.student.studentId' : row.studentId
				}, 
				function() {
