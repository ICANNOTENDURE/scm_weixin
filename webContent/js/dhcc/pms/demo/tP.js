...
$(function (){
	...
	$CommonUI.getDataGrid('#datagrid').datagrid({  
	    url:$WEB_ROOT_PATH+'/demo/tPCtrl!list.htm',
	    iconCls:'icon-edit',//图标
	    method:'post',
	    fit:true,
	    columns:[
            [{"title":"tPId","field":"tPId","sortable":false,"hidden":true},{"title":"pCode","field":"pCode","sortable":false,"hidden":false},{"title":"pName","field":"pName","sortable":false,"hidden":false}]
	    ]	 
	});
	
	//新增或更新成功的回调函数
	function succ(data){
		var Id=$('#detail input[name="dto.tP.tPId"]').val();	
		if(Id){
			$CommonUI.alert("修改成功");
		}else{
			$CommonUI.alert("增加成功");
	...
	$("#submitBtn").on('click', function() {
		...
		postReq($WEB_ROOT_PATH+'/demo/tPCtrl!save.htm',"#detail",succ,err,{skipHidden:false});
	});
...
function editRow() {
	...
	var Id=row.tPId;
	var url = $WEB_ROOT_PATH+'/demo/tPCtrl.htm?BLHMI=findById&dto.tP.tPId='+Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function(){
		...
		$.post($WEB_ROOT_PATH+"/demo/tPCtrl!delete.htm", {'dto.tP.tPId':row.tPId},function(){
