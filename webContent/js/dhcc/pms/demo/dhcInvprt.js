...
$(function (){
	...
	$CommonUI.getDataGrid('#datagrid').datagrid({  
	    url:$WEB_ROOT_PATH+'/demo/dhcInvprtCtrl!list.htm',
	    iconCls:'icon-edit',//图标
	    method:'post',
	    fit:true,
	    columns:[
            [{"title":"dhcInvprtId","field":"dhcInvprtId","sortable":false,"hidden":true},{"title":"发票代码","field":"dhcInvprtCode","sortable":false,"hidden":false},{"title":"发票名称","field":"dhcInvprtName","sortable":false,"hidden":false}]
	    ]	 
	});
	
	//新增或更新成功的回调函数
	function succ(data){
		var Id=$('#detail input[name="dto.dhcInvprt.dhcInvprtId"]').val();	
		if(Id){
			$CommonUI.alert("修改成功");
		}else{
			$CommonUI.alert("增加成功");
	...
	$("#submitBtn").on('click', function() {
		...
		postReq($WEB_ROOT_PATH+'/demo/dhcInvprtCtrl!save.htm',"#detail",succ,err,{skipHidden:false});
	});
...
function editRow() {
	...
	var Id=row.dhcInvprtId;
	var url = $WEB_ROOT_PATH+'/demo/dhcInvprtCtrl.htm?BLHMI=findById&dto.dhcInvprt.dhcInvprtId='+Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function(){
		...
		$.post($WEB_ROOT_PATH+"/demo/dhcInvprtCtrl!delete.htm", {'dto.dhcInvprt.dhcInvprtId':row.dhcInvprtId},function(){
