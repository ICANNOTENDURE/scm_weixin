...
$(function (){
	...
	//新增或更新成功的回调函数
	function succ(data){
		var Id=$('#detail input[name="dto.catGroupProperty.catGroupPropertyId"]').val();	
		if(Id){
			$CommonUI.alert("修改成功");
		}else{
			$CommonUI.alert("增加成功");
	...
	$("#submitBtn").on('click', function() {
		postReq($WEB_ROOT_PATH+'/cat/catGroupPropertyCtrl!save.htm',"#detail",succ,err,{skipHidden:false});
	});
...
function editRow() {
	...
	var Id=row.catGroupPropertyId;
	var url = $WEB_ROOT_PATH+'/cat/catGroupPropertyCtrl.htm?BLHMI=findById&dto.catGroupProperty.catGroupPropertyId='+Id;
...
function delRow() {
	...
	$CommonUI.confirm('确定删除吗？', 'question', 0, function(){
		...
		$.post($WEB_ROOT_PATH+"/cat/catGroupPropertyCtrl!delete.htm", {'dto.catGroupProperty.catGroupPropertyId':row.catGroupPropertyId},function(){
