...
<script>
 	$(function (){
 		$("#upload").uploadify({
 	        ...
 	        'uploader': $WEB_ROOT_PATH + '/ven/venDeliverCtrl!uploadInvAndroid.htm',
 	       ...
 	       'onUploadStart': function(file) {
 	        	$("#upload").uploadify("settings", 'formData', {'dto.uploadFileName': file.name});
 	        },
 	        'onSelect': function(){  
 	        	$("#gg").dialog("open");
 	        	$("#err").html("");
 		...
 		$("#delete").on('click', function() {
 			...
 			if(selected[0].delvSendFlag==1){
 				$CommonUI.alert("已发货,不能删除");
 	            return;
 			}
 			$.post(
 					 $WEB_ROOT_PATH+'/ven/venDeliverCtrl!deleteImpInv.htm',
 					 {
 						 "dto.impId":selected[0].delvId,
 					 },
 					 function(data){
		...
		$("#send").on('click', function() {
			...
			$.post(
					 $WEB_ROOT_PATH+'/ven/venDeliverCtrl!sendImpInv.htm',
					 {
						 "dto.impId":selected[0].delvId,
					 },
					 function(data){
 				...
 				onClickRow:function(rowIndex, rowData){
 					$CommonUI.getDataGrid('#datagrid2').datagrid({  
 				 	...
 				 	$("#datagrid2").datagrid({
 			 			url:$WEB_ROOT_PATH+'/ven/venDeliverCtrl!listDeliverItmNew.htm',
 						queryParams: {
 							'dto.impId': $('#datagrid').datagrid('getRows')[0]['delvId'],
 							}
 		 				}
 				 	);
 			 	 },
...
function deleterow(index){
	...
	$.messager.confirm('确认对话框', '确认要删除吗？', function(r){
		if (r){
			...
			$.post(
					   $WEB_ROOT_PATH + '/ord/orderStateCtrl!deleteOrdSub.htm', 
		    			{
		    				"dto.orderDetailSub.ordSubId" : deliveritmid,
		    			}, 
		    			function(data) {
		    				if (data.resultCode=="1") {
...
function onAfterEdit(rowIndex, rowData, changes){
	 ...
	 $.post(
			 $WEB_ROOT_PATH+'/ord/orderStateCtrl!saveOrdSub.htm',
			 {
				 "dto.orderDetailSub.ordSubId":rowData.deliveritmid,
	          	 ...
	          	 "dto.orderDetailSub.orderSubQty":rowData.deliverqty,
	          	 "dto.orderDetailSub.ordSubDetailId":rowData.orderitmid
			 },
			 function(data){
				 if(data.resultCode=="1"){					 
...
<body>
	...
	<div id="toolbar" class="toolbar">
			...
			<a class="linkbutton" id="delete"
				data-options="iconCls:'icon-remove',plain:true">删除</a>
			<a class="linkbutton" id="send"
	...
	<div class="layout" data-options="fit:'true',border:true">
		<div data-options="region:'north',title:'上传发票流水(双击查看明细)',iconCls:'icon-ok'"
			...
			<table id="datagrid"  class="datagrid"
				...
				<thead>
					<tr>
						<th data-options="field:'delvId',hidden:true">流水号</th>
						<th data-options="field:'delvNo',width:100">流水号</th>
						<th data-options="field:'delvDate',width:100">时间</th>
						<th data-options="field:'delvSendFlag',width:100">状态</th>
					</tr>
				</thead>
			</table>
				...
				<thead>
					<tr>
					   ...
					   ">进价</th>
						...
						<th data-options="field:'spec',width:30,hidden:true">规格</th>
