...
<script type="text/javascript">
	$(function() {
		$(".deliver-history-list li")
				.on(
						'click',
						function() {
							window.location.href = $WEB_ROOT_PATH
									+ "/weixin/wxMessageCtrl!mbListOrderDetail.htm?dto.orderDetail.orderNo="
...
<body>
	...
	<form id="subscribe-form"
		action="<%=request.getContextPath()%>/weixin/wxMessageCtrl!listToDoTask.htm"
		method="post">
		    <input type="hidden" name="dto.start" value="${dto.start}" id="qurStart"/>
		    <input type="hidden" name="dto.end" value="${dto.end}" id="qurEnd"/>
		    <input type="hidden" name="dto.state" value="${dto.state}" id="qurState"/> 		
	...
	<div data-am-widget="tabs" class="am-tabs am-tabs-d2">
		<ul class="am-tabs-nav am-cf">
			...
			<li class=""><a href="[data-tab-panel-1]">加急</a></li>
		</ul>
		<div class="am-tabs-bd">
			<div data-tab-panel-0 class="am-tab-panel am-active">
				<ul class="deliver-history-list">
					<s:iterator value="dto.orderInfos" status="status" id="orderInfo">
						<li ng-repeat="item in jobList"><header class="ellipsis">
								<h1 class="title ng-binding">${orderInfo.orderno}</h1>
							</header>
							<div class="status ng-binding">${orderInfo.state}</div>
							<p class="ellipsis ng-binding">
								<span class="salary ng-binding">${orderInfo.hopname}</span>|
								${orderInfo.recdesction}
							</p>
							<p class="time ng-binding">${orderInfo.orddate}</p></li>
					</s:iterator>
				</ul>
			...
			<div data-tab-panel-1 class="am-tab-panel ">
				<ul class="deliver-history-list">
					<li ng-repeat="item in jobList"></li>
				</ul>
			</div>
		</div>
	</div>
